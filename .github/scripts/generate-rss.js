  ...
  for (const file of files) {
    fs.readFile(file, "utf8", function (err, data) {
      ...
      for (const entry of entries) {
        ...
        const existingEntryIndex = oldFeedItems.findIndex(
          (item) => item.guid === newEntry.guid
        );

        if (
          existingEntryIndex === -1 ||
          !entriesAreEqual(oldFeedItems[existingEntryIndex], newEntry)
        ) {
...
if (isContentChanged) {
  ...
  newXMLContent = newXMLContent.replace(
    ...
    `<lastBuildDate>${newLastBuildDate}</lastBuildDate>`
  );
  newXMLContent = newXMLContent.replace(/{target=&quot;_blank&quot;}/g, "");
  newXMLContent = newXMLContent.replace(
    /\.\.\/static\//g,
    "https://fcp.cafe/static/"
  );
